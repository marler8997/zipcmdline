...
pub fn main() !void {
    ...
    if (false) {
        ...
        var writer = zip_file.writer(&write_buffer);
...
fn writeZip(
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // TODO: what's a good size for this?
    // var write_buffer: [4096]u8 = undefined;
    var write_buffer: [1]u8 = undefined;
    var zipper: Zipper = .init(&write_buffer, writer);

    for (file_entries, 0..) |file_entry, i| {
        const file_offset = zipper.getBytesWritten();
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        std.log.info("Entry {}: FileOffset={}", .{ i, file_offset });
        ...
        switch (compression) {
            ...
            .deflate => {
                ...
                // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                std.log.info("  StartOffset={}", .{start_offset});
                ...
                std.log.info("  EndOffset={} (size={}) crc=0x{x}", .{ zipper.getBytesWritten(), compressed_size, crc32 });
            },
            else => @panic("codebug"),
        }
        store[i] = .{
...
const Crc32Reader = struct {
    ...
    fn readVec(r: *std.Io.Reader, data: [][]u8) std.Io.Reader.Error!usize {
        const self: *Crc32Reader = @fieldParentPtr("interface", r);
        ...
        while (bytes_remaining != 0) : (data_index += 1) {
            ...
            // std.log.info(
            //     "  remaining={} data[{}].len={} (next_hash_len={})",
            //     .{ bytes_remaining, data_index, data[data_index].len, next_hash_len },
            // );
            self.crc32.update(data[data_index][0..next_hash_len]);
            bytes_remaining -= next_hash_len;
        }
        // std.log.info("returning {}", .{len});
        return len;
    }
...
const Zipper = struct {
    ...
    pub fn init(write_buffer: []u8, underlying_writer: *std.Io.Writer) Zipper {
        return .{
    ...
    fn drain(w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
        const self: *Zipper = @fieldParentPtr("writer", w);
